...
class PlayPlayerByPositionHandler(AbstractRequestHandler):
    ...
    def handle(self,handler_input):
        ...
        skill_name = language_prompts["SKILL_NAME"]
        number = int(handler_input.request_envelope.request.intent.slots["number"].slot_value.value)
        ranking_list = json.loads(http('/prod/players/ranking/{}'.format(number)))
        len_ranking_list = len(ast.literal_eval(ranking_list))
        speech_output = '{}'.format(len_ranking_list) #'{language_prompts["TOP_PLAYER"][len_ranking_list-1]}'.format(number)
